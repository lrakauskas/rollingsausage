...
- Events are emitted from native Android and bridged to PHP.
- Keep listeners idempotent where possible.
- Prefer explicit `load*()` then `show*()` lifecycle handling for interstitial and rewarded flows.
